  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 md:px-8 py-10">
      <div className="flex flex-wrap items-end justify-between gap-6">
        ...
        <div className={`flex gap-2 rounded-full p-1 border ${isDark? 'bg-[#071726] border-white/10' : 'bg-white border-black/5'}`}>{['all','sale','rent'].map(s=><button key={s} onClick={()=>setFilters({...filters,status:s})} className={`rounded-full px-5 py-2 text-[11px] uppercase font-black tracking-widest transition-luxe ${filters.status===s?'bg-brick-black text-white': isDark? 'text-white/80 hover:text-white' : 'text-brick-muted hover:text-black'}`}>{s}</button>)}</div>
      </div>
