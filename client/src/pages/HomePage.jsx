...
export default function HomePage(){
  ...
  return (
    <div className={`min-h-screen transition-colors ${isDark? 'bg-[#081714] text-white' : 'bg-brick-offwhite text-brick-charcoal'}`}>
      <HeroSearch onSearch={setFilter}/>
      <TrustBar/>
      <section className="mx-auto max-w-[1400px] px-6 md:px-8 py-16 md:py-24">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] items-start">
          <div className={`rounded-[32px] p-8 md:p-10 border shadow-soft sm:sticky top-[88px] ${isDark? 'bg-[#071726] border-white/10 text-white' : 'bg-white/80 border-black/5 text-[#081714]'}`}>
            ...
            <p className={`mt-5 text-[15px] leading-7 ${isDark? 'text-white/80' : 'text-brick-muted'}`}>Cleanly presented homes, direct communication, less friction.</p>
            <Link to="/properties" className={`mt-8 inline-flex items-center gap-2 rounded-full px-5 py-3 text-[11px] font-black uppercase tracking-[0.24em] transition-colors ${isDark? 'bg-white text-brick-black hover:bg-white/90' : 'bg-brick-black text-white hover:bg-brick-gold'}`}>
      ...
      <section className={`py-16 md:py-24 ${isDark? 'bg-[#071726]' : 'bg-white'}`}>
